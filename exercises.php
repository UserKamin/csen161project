...
// Load the HTML file
$htmlFile = 'exercises.html';
if (!file_exists($htmlFile)) {
    echo "Error: exercises.html file not found.";
    exit;
}
...
foreach ($exercises as $exercise) {
    ...
    $exerciseElement->setAttribute('class', 'exercise-card');
    
    // Add muscle groups and difficulty as data attributes for filtering
    $exerciseElement->setAttribute('data-muscle-groups', strtolower($exercise['muscle_groups']));
    $exerciseElement->setAttribute('data-difficulty', strtolower($exercise['difficulty']));

    // Exercise name
    $nameElement = $doc->createElement('h2', htmlspecialchars($exerciseName));

    // Muscle groups as tags
    $muscleGroupsArray = explode(', ', $muscleGroups); // Split the muscle groups string into an array
    $muscleGroupsContainer = $doc->createElement('div');
    $muscleGroupsContainer->setAttribute('class', 'muscle-groups-container');

    foreach ($muscleGroupsArray as $muscleGroup) {
        $muscleGroupTag = $doc->createElement('div');
        $muscleGroupTag->setAttribute('class', 'muscle-group-tag');
        $muscleGroupTag->appendChild($doc->createTextNode(trim($muscleGroup)));
        $muscleGroupsContainer->appendChild($muscleGroupTag);
    }
    ...
    // Link to how to perform the exercise (styled as a button)
    $linkElement = $doc->createElement('button', 'How to Perform');
    $linkElement->setAttribute('class', 'btn');
    $linkElement->setAttribute('onclick', "window.open('$link', '_blank')");

    // Add to Workout button
    $addToWorkoutButton = $doc->createElement('button', 'Add to Workout');
    $addToWorkoutButton->setAttribute('class', 'btn');
    ...
    $exerciseElement->appendChild($muscleGroupsContainer);
...
if (!empty($workoutPlans)) {
    foreach ($workoutPlans as $workout) {
        ...
        $workoutPlanButton->setAttribute('class', 'btn');
