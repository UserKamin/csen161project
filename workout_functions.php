...
// Function to save workouts manually
function saveWorkouts() {
    ...
    if (empty($workoutsData)) {
        echo json_encode(['success' => false, 'message' => 'Workouts data is required']);
        return;
    }
    ...
    try {
        // Update workouts in database
        $stmt = $pdo->prepare("UPDATE users SET workouts = ? WHERE user_id = ?");
        $stmt->execute([$workoutsData, $userId]);
